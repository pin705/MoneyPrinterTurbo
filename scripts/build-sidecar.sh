...
RENDER="$ROOT/apps/render"   # the render backend lives here
...
# Build the sidecar from the render backend (spec datas/main.py are relative).
( cd "$RENDER" && pyinstaller --clean -y packaging/mpt-backend.spec )
...
cp "$RENDER/dist/mpt-backend${EXT}" "$DEST/mpt-backend-${TRIPLE}${EXT}"
