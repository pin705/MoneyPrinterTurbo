...
export default defineConfig({
  ...
  webServer: [
    {
      // Force dev sign-in (deterministic; no real Google OAuth) even if a local
      // .env has Supabase creds.
      command: `VITE_FORCE_DEV_AUTH=1 pnpm build && pnpm preview --port ${WEB} --strictPort`,
