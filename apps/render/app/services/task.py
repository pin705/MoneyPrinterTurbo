...
def generate_script(task_id, params):
    ...
    if not video_script:
        # Phase 4: ground the script in real facts (free, Wikipedia). Appended to
        # the user's extra requirements; never blocks the render (best-effort).
        extra_prompt = params.video_script_prompt or ""
        if config.app.get("research", True):
            try:
                facts = research.gather_facts(
                    params.video_subject, params.video_language
                )
            except Exception as e:
                logger.warning(f"research grounding skipped: {e}")
                facts = ""
            if facts:
                extra_prompt = (
                    extra_prompt
                    + "\n\n# Reference facts (ground the script in these; do not invent):\n"
                    + facts
                ).strip()
